...
# Lou: we import the generic stop_words library
data(stop_words)

# find document-word counts and remove stop words
# Lou: word_counts should be 52,216 rows
word_counts <- by_lesson_word %>% 
  anti_join(stop_words) %>% 
  count(title, word, sort = TRUE) %>% 
  ungroup()

# Lou: word_counts should have 51,033 rows
word_counts <- word_counts %>% arrange(word) %>% slice(-1:-1183)

#########################################
# Do this after my_stop_words is created (see below) 
# Lou: THIS MUST BE RUN TWICE!!!! Don't ask me why.
# Lou: word_counts still has 51,033 rows
